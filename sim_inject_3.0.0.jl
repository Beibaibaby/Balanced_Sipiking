...
@everywhere function sim_dynamic_parallelized(Ne,Ni,T,taue,taui,pei,pie,pii,pee,K,stimstr_para,Nstim,jie_para,
    ...
    xerise = SharedArray(xerise)
    xedecay = SharedArray(xedecay)
    xirise = SharedArray(xirise)
    xidecay = SharedArray(xidecay)
    E_input = SharedArray(E_input)
    I_input = SharedArray(I_input)
    v = SharedArray(v)
    v_history = SharedArray(v_history)
    lastSpike = SharedArray(lastSpike)
    ns = SharedArray(ns)
    times = SharedArray(times)
    weights_D = SharedArray(weights_D)
    weights_F = SharedArray(weights_F)
    forwardInputsE = SharedArray(forwardInputsE)
    forwardInputsI = SharedArray(forwardInputsI)
